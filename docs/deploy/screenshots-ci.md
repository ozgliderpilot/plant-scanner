...
cull. Walk order: Actions (Sell → Cull → Print → Repot) → History (Receipts → Culls → Labels →
Repots) → Plants. Walked sale is `1001` pot @ `$5.00` (−10%) plus `1003` pot @ `$4.00` before
confirm (no extra frames). A second same-day sale (`1002` @ `$8.00`) runs after confirm so receipts
